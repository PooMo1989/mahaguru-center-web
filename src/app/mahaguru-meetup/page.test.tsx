...
describe('MahaguruMeetupPage', () => {
  it('renders the page title correctly', () => {
    ...
    expect(screen.getByRole('heading', { name: /book a mahaguru meetup/i })).toBeInTheDocument();
  });
  ...
  it('renders all six required sections', () => {
    ...
    expect(screen.getByRole('heading', { name: /about mahaguru/i })).toBeInTheDocument();
    
    // Section 2: What is a Mahaguru Meetup
    expect(screen.getByTestId('section-what-is-meetup')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: /your moment of absolute clarity/i })).toBeInTheDocument();
    
    // Section 3: Booking Options
    expect(screen.getByTestId('section-booking-options')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: /booking options/i })).toBeInTheDocument();
    
    // Section 4: Group & Team Sessions
    expect(screen.getByTestId('section-group-team-sessions')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: /group & team sessions/i })).toBeInTheDocument();
    
    // Section 5: Our Mission
    expect(screen.getByTestId('section-our-mission')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: /our mission \(contribution\)/i })).toBeInTheDocument();
    
    // Section 6: Free Alternative
    expect(screen.getByTestId('section-free-alternative')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: /experience the dialogue/i })).toBeInTheDocument();
  });
  ...
  it('displays all required content from Project Brief', () => {
    ...
    // Section 1 content
    expect(screen.getByText(/a rare human being who has walked a unique spiritual path for over 20 years/i)).toBeInTheDocument();
    
    // Section 2 content
    expect(screen.getByText(/in a world of endless noise, a mahaguru meetup is a space of profound stillness/i)).toBeInTheDocument();
    ...
    expect(screen.getByText(/your payment is an offering that directly supports the sustainability/i)).toBeInTheDocument();
    
    // Section 6 free alternative content
    expect(screen.getByText(/not sure where to begin\? join our weekly recorded group consultation/i)).toBeInTheDocument();
  });
  ...
  it('renders all call-to-action buttons', () => {
    ...
    // Verify button text
    expect(screen.getByRole('link', { name: /book in-person session/i })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /book online session/i })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /inquire for your team/i })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /inquire for your group/i })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /learn more about our mission/i })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /register for free session/i })).toBeInTheDocument();
  });
