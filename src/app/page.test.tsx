...
describe("Homepage UI Enhancements (Story 7.1)", () => {
  it("renders the full-screen hero section with background image", () => {
    ...
    expect(screen.getByTestId("image-mahaguru-center")).toHaveAttribute("src", "/heroImage2.webp");
  });
  ...
  it("renders the About Mahaguru section with proper layout", () => {
    ...
    expect(screen.getByText(/A rare human being who has walked a unique spiritual path/)).toBeInTheDocument();
    ...
    expect(screen.getByTestId("image-mahaguru")).toHaveAttribute("src", "/Untitled design (1).jpg");
  });
  ...
  it("renders the Our Services section", () => {
    ...
    expect(screen.getByText(/From Dhamma Talks to Individual meetups/)).toBeInTheDocument();
    expect(screen.getByText("Explore")).toBeInTheDocument();
    
    // Services image
    expect(screen.getByTestId("image-our-services")).toBeInTheDocument();
    expect(screen.getByTestId("image-our-services")).toHaveAttribute("src", "/Screenshot 2025-08-20 185255.jpg");
  });
  ...
  it("renders the Monthly Dhamma Discussion section", () => {
    ...
    expect(screen.getByText(/We had been continuously conducting Monthly Dhamma Discussion/)).toBeInTheDocument();
    expect(screen.getByText("View Events")).toBeInTheDocument();
    
    // Events image
    expect(screen.getByTestId("image-monthly-dhamma-discussion")).toBeInTheDocument();
    expect(screen.getByTestId("image-monthly-dhamma-discussion")).toHaveAttribute("src", "/Screenshot 2025-07-29 161148.jpg");
  });
  ...
  it("renders the Our Projects section with image grid", () => {
    ...
    expect(screen.getByText(/We have several ongoing physical and digital projects/)).toBeInTheDocument();
    expect(screen.getByText("See Projects")).toBeInTheDocument();
    
    // Project images
    expect(screen.getByTestId("image-project-1")).toBeInTheDocument();
    expect(screen.getByTestId("image-project-1")).toHaveAttribute("src", "/Screenshot 2025-08-20 223553.jpg");
    expect(screen.getByTestId("image-project-2")).toBeInTheDocument();
    expect(screen.getByTestId("image-project-2")).toHaveAttribute("src", "/490061273_1097783239058409_8966922447246500945_n.jpg");
  });
