...
describe("Gen Alpha Academy Landing Page", () => {
  ...
  describe("Section 1: Header", () => {
    it("displays the hero section with title, subtitle, and Reserve Now button", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "Gen Alpha Academy" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", {
          name: "Cultivating Leaders & Wise Minds for the 21st Century",
        }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("link", { name: "Reserve Now" }),
      ).toBeInTheDocument();
    });
  ...
  describe("Section 2: Workshop Introduction", () => {
    it("displays the workshop introduction with left/right split layout", () => {
      ...
      expect(
        screen.getByRole("heading", {
          name: "Fundamentals for Lifelong Transformation",
        }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", { name: "Dear Parents and Young Adults," }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
      ...
      expect(
        screen.getByText(/transformative 5-hour experience filled with energy/),
      ).toBeInTheDocument();
    });
    ...
    it("displays the complete workshop introduction text", () => {
      ...
      expect(
        screen.getByText(/Whether you're exploring who you are/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/help you see your future more clearly/),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Section 3: Four Pillars", () => {
    it("displays the four pillars section header and introduction", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "What makes the program special" }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          /This is the stepping stone to a comprehensive life-preparation journey/,
        ),
      ).toBeInTheDocument();
    });
    ...
    it("displays all four pillars with exact text from Project Brief", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "Self-Realization" }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
      ...
      expect(
        screen.getByRole("heading", { name: "Maturity through wisdom" }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
      ...
      expect(
        screen.getByRole("heading", { name: "Leadership Training" }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
      ...
      expect(
        screen.getByRole("heading", { name: "Emotional Management" }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
  ...
  describe("Section 4: Program Details & Inclusions", () => {
    it("displays program details with all specified information", () => {
      ...
      expect(
        screen.getByText(/August 22nd \(Sunday – During School Holidays\)/),
      ).toBeInTheDocument();
      expect(screen.getByText(/5 Hours/)).toBeInTheDocument();
      expect(screen.getByText(/15–22 Years/)).toBeInTheDocument();
      expect(
        screen.getByText(/Limited to 15 Participants Only/),
      ).toBeInTheDocument();
      expect(screen.getByText(/Sinhala/)).toBeInTheDocument();
      expect(
        screen.getByText(/Organized by the Mahaguru Center Australia/),
      ).toBeInTheDocument();
    });
    ...
    it("displays what's included section", () => {
      ...
      expect(
        screen.getByText(/Comprehensive training materials and handouts/),
      ).toBeInTheDocument();
      expect(screen.getByText(/Meals for the day/)).toBeInTheDocument();
      expect(
        screen.getByText(/Access to follow-up sessions monthly/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Access to recorded sessions/),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Section 5: Investment & Mission", () => {
    it("displays investment details", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "Investment in Your Future" }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Program Contribution: LKR 5,000 per participant/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/This fee supports high-quality delivery/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/If you are unable to make the payment/),
      ).toBeInTheDocument();
    ...
    it("displays mission contribution explanation", () => {
      ...
      expect(
        screen.getByRole("heading", {
          name: "What happens to your contribution",
        }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Your payment is an offering that directly supports/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/maintain our physical space/),
      ).toBeInTheDocument();
    });
    ...
    it("has Learn More About Our Mission link with proper attributes", () => {
      ...
      const missionLink = screen.getByRole("link", {
        name: "Learn More About Our Mission →",
      });
  ...
  describe("Section 6: Why This Program Matters", () => {
    it("displays the program importance explanation", () => {
      ...
      expect(
        screen.getByText(
          /In today's world, young people need skills that support long-term success/,
        ),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          /This program is designed to plant seeds of awareness/,
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Section 7: Final Call to Action", () => {
    it("displays the final registration call to action", () => {
      ...
      expect(
        screen.getByText(/Register Now – Limited Seats!/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
  ...
  describe("External Links and Call-to-Action Buttons", () => {
    it("displays at least two call-to-action buttons", () => {
      ...
      const reserveButtons = screen.getAllByRole("link", {
        name: "Reserve Now",
      });
      ...
      expect(
        screen.getByRole("link", { name: "Inquire via WhatsApp" }),
      ).toBeInTheDocument();
  ...
  describe("Content Verification", () => {
    it("does NOT display 'Coming Soon' content from previous version", () => {
      ...
      expect(
        screen.queryByText(/Program Details Coming Soon/),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(
          /We're currently finalizing the specific workshop details/,
        ),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(/Stay tuned for announcements/),
      ).not.toBeInTheDocument();
    });
    ...
    it("has all 7 sections from Project Brief", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "Gen Alpha Academy" }),
      ).toBeInTheDocument(); // Section 1
      expect(
        screen.getByRole("heading", {
          name: "Fundamentals for Lifelong Transformation",
        }),
      ).toBeInTheDocument(); // Section 2
      expect(
        screen.getByRole("heading", { name: "What makes the program special" }),
      ).toBeInTheDocument(); // Section 3
      expect(screen.getByText(/Program Details/)).toBeInTheDocument(); // Section 4
      expect(
        screen.getByRole("heading", { name: "Investment in Your Future" }),
      ).toBeInTheDocument(); // Section 5
      // Section 6 has no heading but content is tested elsewhere
      expect(
        screen.getByText(/Register Now – Limited Seats!/),
      ).toBeInTheDocument(); // Section 7
    });
  });
});
