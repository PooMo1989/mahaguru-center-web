...
describe("ProjectForm", () => {
  ...
  it("renders create form with all required fields", () => {
    ...
    expect(screen.getByLabelText(/current donation amount/i)).toBeInTheDocument();
  ...
  it("renders edit form with populated data", () => {
    ...
    expect(screen.getByDisplayValue("Digital Infrastructure")).toBeInTheDocument();
  ...
  it("validates donation amounts as positive numbers", async () => {
    ...
    const currentAmountInput = screen.getByLabelText(/current donation amount/i);
    ...
    fireEvent.change(screen.getByLabelText(/project type/i), { target: { value: "Test Type" } });

    const submitButton = screen.getByRole("button", { name: /create project/i });
    fireEvent.click(submitButton);

    await waitFor(() => {
      expect(screen.getByText("Donation goal amount must be positive")).toBeInTheDocument();
    }, { timeout: 3000 });
    
    await waitFor(() => {
      expect(screen.getByText("Current donation amount cannot be negative")).toBeInTheDocument();
    }, { timeout: 3000 });
  });
