...
const projectSchema = z.object({
  ...
  currentDonationAmount: z.number().min(0, "Current donation amount cannot be negative"),
...
export function ProjectForm({ 
  ...
  const [formData, setFormData] = useState({
    ...
    startDate: "",
    endDate: "",
    donationLinkTarget: "Daily Dana" as "Daily Dana" | "Poya Day" | "Special Projects",
  });
  ...
  useEffect(() => {
    if (mode === "edit" && editProject) {
      ...
      setFormData({
        ...
        donationGoalAmount: typeof editProject.donationGoalAmount?.toNumber === 'function' 
          ? editProject.donationGoalAmount.toNumber() 
          : Number(editProject.donationGoalAmount) || 0,
        currentDonationAmount: typeof editProject.currentDonationAmount?.toNumber === 'function' 
        ...
        donationLinkTarget: editProject.donationLinkTarget as "Daily Dana" | "Poya Day" | "Special Projects",
      });
    }
  }, [mode, editProject]);
  ...
  const validateForm = () => {
    const transformedData = {
      ...
      donationGoalAmount: typeof formData.donationGoalAmount === 'string' && formData.donationGoalAmount === '' 
        ? 0 
        : Number(formData.donationGoalAmount),
      currentDonationAmount: typeof formData.currentDonationAmount === 'string' && formData.currentDonationAmount === '' 
        ? 0 
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          <div className="flex items-center">
            ...
            />
            <label htmlFor="continuous" className="ml-3 block text-sm font-medium text-gray-700">
              Continuous
            </label>
          </div>
          <div className="flex items-center">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div>
          <label htmlFor="donationGoalAmount" className="block text-sm font-medium text-gray-700">
          ...
          {errors.donationGoalAmount && (
            <p className="mt-1 text-sm text-red-600">{errors.donationGoalAmount}</p>
          )}
        </div>

        <div>
          <label htmlFor="currentDonationAmount" className="block text-sm font-medium text-gray-700">
          ...
          {errors.currentDonationAmount && (
            <p className="mt-1 text-sm text-red-600">{errors.currentDonationAmount}</p>
          )}
        </div>
      </div>
      ...
      <div>
        <label htmlFor="donationLinkTarget" className="block text-sm font-medium text-gray-700">
          Donation Link Target *
        </label>
        <select
        ...
        {errors.donationLinkTarget && (
          <p className="mt-1 text-sm text-red-600">{errors.donationLinkTarget}</p>
        )}
      </div>
