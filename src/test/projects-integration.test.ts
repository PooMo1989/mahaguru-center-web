...
function filterProjectsByNature(
  projects: TestProject[],
  filter: "all" | "Continuous" | "One-time",
) {
  if (filter === "all") return projects;
  return projects.filter((project) => project.projectNature === filter);
}

function filterProjectsByDonationTarget(
  projects: TestProject[],
  filter: "all" | "Daily Dana" | "Poya Day" | "Special Projects",
) {
  if (filter === "all") return projects;
...
const projectCreateInputSchema = z
  .object({
...
describe("Project Input Validation", () => {
  it("rejects invalid project creation data", () => {
    const invalidData = {
      ...
      projectNature: "Invalid", // Invalid enum value
      donationLinkTarget: "Daily Dana" as const,
    };
  ...
  it("validates projectNature enum values", () => {
    ...
    expect(
      continousSchema.safeParse(validContinuous.projectNature).success,
    ).toBe(true);
    expect(continousSchema.safeParse(validOneTime.projectNature).success).toBe(
      true,
    );
    expect(continousSchema.safeParse(invalid.projectNature).success).toBe(
      false,
    );
  });
  ...
  it("validates donationLinkTarget enum values", () => {
    ...
    expect(
      targetSchema.safeParse(validDailyDana.donationLinkTarget).success,
    ).toBe(true);
    expect(
      targetSchema.safeParse(validPoyaDay.donationLinkTarget).success,
    ).toBe(true);
    expect(
      targetSchema.safeParse(validSpecialProjects.donationLinkTarget).success,
    ).toBe(true);
    expect(targetSchema.safeParse(invalid.donationLinkTarget).success).toBe(
      false,
    );
  });
  ...
  it("validates DECIMAL field handling", () => {
    const validDecimalData = {
      ...
      donationGoalAmount: 1500.5, // Should be converted to Decimal
  ...
  it("validates date constraint (end date after start date)", () => {
    ...
    expect(projectCreateInputSchema.safeParse(validDateData).success).toBe(
      true,
    );
    expect(projectCreateInputSchema.safeParse(invalidDateData).success).toBe(
      false,
    );
  });
  ...
  it("validates photo URLs array", () => {
    ...
    expect(projectCreateInputSchema.safeParse(validPhotoData).success).toBe(
      true,
    );
    expect(projectCreateInputSchema.safeParse(invalidPhotoData).success).toBe(
      false,
    );
  });
});
...
describe("Project Filtering Logic", () => {
  ...
  it("filters projects by nature correctly", () => {
    const continuousProjects = filterProjectsByNature(
      mockProjects,
      "Continuous",
    );
  ...
  it("filters projects by donation link target correctly", () => {
    const dailyDanaProjects = filterProjectsByDonationTarget(
      mockProjects,
      "Daily Dana",
    );
    const specialProjectsProjects = filterProjectsByDonationTarget(
      mockProjects,
      "Special Projects",
    );
    ...
    expect(dailyDanaProjects[0]?.projectName).toBe(
      "Arahathmaga Spiritual Center",
    );
